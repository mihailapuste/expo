...
type TrackRemoteEventName = 'onRemoteNextTrack' | 'onRemotePreviousTrack';

describe('AudioLockScreenOptions', () => {
  it('accepts Android and iOS next and previous track controls with existing lock screen controls', () => {
    const options = {
  ...
  it('types the remote command events emitted from iOS lock screen controls', () => {
    const events: string[] = [];
    const handlers: Pick<AudioEvents, IosRemoteEventName> = {
      onRemotePlay() {
        events.push('play');
      },
      onRemotePause() {
    ...
    expect(events).toEqual(['play', 'pause', 'toggle', 'forward:10', 'backward:10', 'seek:42']);
  });

  it('types the track command events emitted from Android and iOS lock screen controls', () => {
    const events: string[] = [];
    const handlers: Pick<AudioEvents, TrackRemoteEventName> = {
      onRemoteNextTrack() {
        events.push('next');
      },
